<script>
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      dispatch('error', {
        title: 'Error',
        message: `Failed to create repository: ${error.message || error}`
      });
    } finally {
      isLoading = false;
    }
...
{#if isOpen}
  <div class="modal modal-open" style="z-index: 1000;">
    <div class="modal-box max-w-md">
      <h3 class="font-bold text-lg mb-6 text-primary">Add New Repository</h3>
      ...
      <form on:submit|preventDefault={handleSubmit}>
        ...
        <!-- Description -->
        <div class="form-control w-full mb-4">
          <label class="label" for="repo-description">
        ...
        <div class="form-control w-full mb-6">
          <label class="label" for="repo-path">
          ...
          {:else if formData.path.trim() && !isValidPath}
            <div class="label">
              <span class="label-text-alt text-error">⚠ Path Not Valid!</span>
            </div>
          {/if}
        </div>
